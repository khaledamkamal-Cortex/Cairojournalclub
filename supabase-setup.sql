-- ============================================================================
-- CAIRO Journal Club portal — COMPLETE Supabase setup
-- Run this ONCE in the Supabase SQL editor (Dashboard → SQL Editor → New query
-- → paste all of this → Run). Safe to re-run: uses IF NOT EXISTS / guards.
--
...
declare pol record;
begin
  -- Nothing to pre-drop on first run; re-runs handled by the drops below.
  null;
end $$;

-- Public read of published content
drop policy if exists "read events"     on events;      create policy "read events"     on events      for select using (true);
drop policy if exists "read materials"  on materials;   create policy "read materials"  on materials   for select using (true);
drop policy if exists "read news"       on news;        create policy "read news"       on news        for select using (true);
...
drop policy if exists "admin events"    on events;      create policy "admin events"    on events      for all using (is_admin()) with check (is_admin());
drop policy if exists "admin materials" on materials;   create policy "admin materials" on materials   for all using (is_admin()) with check (is_admin());
drop policy if exists "admin news"      on news;        create policy "admin news"      on news        for all using (is_admin()) with check (is_admin());
...
-- Commentaries: public reads approved; anyone submits (pending); admins see & moderate all
drop policy if exists "read approved commentaries" on commentaries; create policy "read approved commentaries" on commentaries for select using (approved = true or is_admin());
drop policy if exists "submit commentary"          on commentaries; create policy "submit commentary"          on commentaries for insert with check (true);
drop policy if exists "admin update commentary"    on commentaries; create policy "admin update commentary"    on commentaries for update using (is_admin());
drop policy if exists "admin delete commentary"    on commentaries; create policy "admin delete commentary"    on commentaries for delete using (is_admin());
...
-- ============================================================================
-- Seed: the two CAIRO LMS courses (only if no courses exist yet).
...
begin
  ...
  insert into courses(title, level, summary, banner, hours, sort) values (
    ...
    'The core CAIRO curriculum: learn to read, appraise and apply oncology randomized controlled trials. Covers study design, bias, endpoints, statistics and applicability.',
    'linear-gradient(135deg,#6d2077,#1b9cd8)', 6, 1) returning id into c;
...
The 95% confidence interval (CI) tells you the precision. If the CI for an HR crosses 1.0, the result is not statistically significant. A wide CI signals an underpowered or small trial. Never read the point estimate without its interval.$t$, 2);
  ...
  insert into courses(title, level, summary, banner, hours, sort) values (
    ...
    'linear-gradient(135deg,#00a896,#7cb518)', 4, 2) returning id into c;
...
--   1. Go to the live site → Membership → Register your own account.
--      (This creates your Supabase Auth user + members row.)
